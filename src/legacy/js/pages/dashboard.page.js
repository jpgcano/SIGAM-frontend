...
(function () {
  ...
  class DashboardPage extends BasePage {
    ...
    async loadDashboardData() {
      const recentContainer = this.qs('#recentTickets');
      const maintenanceContainer = this.qs('#upcomingMaintenance');
      ...
      const [activos, tickets, mantenimientos] = await Promise.all([
        this.safeFetch(this.api.getActivos),
        this.safeFetch(this.api.getTickets),
        this.safeFetch(this.api.getMantenimientos)
      ]);

      const data = this.buildDashboardData(activos, tickets, mantenimientos);
      this.updateCards(data);
      this.createCharts(data);

      if (recentContainer) {
        this.renderRecentTickets(recentContainer, (tickets || []).map((item) => this.normalizeTicket(item)));
      }
      if (maintenanceContainer) {
        this.renderUpcomingMaintenance(maintenanceContainer, mantenimientos);
      }
    }
    ...
    renderRecentTickets(container, list) {
      ...
      recent.forEach((ticket) => {
        ...
        const ui = this.mapStatus(ticket.status);
        item.innerHTML = `
    ...
    buildDashboardData(activos, tickets, mantenimientos) {
      ...
      const scheduledMaintenance = this.countUpcoming(safeMaint, 30);
      ...
      return {
        ...
        assetsByType,
        ticketsByCategory,
        maintenanceCost: this.buildMaintenanceTrend(safeMaint)
      };
    }

    updateCards(data) {
      const assetsEl = this.qs('.card-activos h2');
      const ticketsEl = this.qs('.card-tikects h2');
      const scheduledEl = this.qs('.card-scheduled h2');
      const costEl = this.qs('.card-cost h2');
    ...
    createPieChart(data) {
      const footChart = this.qs('#footChart');
      if (!footChart) return;
      if (this.pieChartInstance) {
        this.pieChartInstance.destroy();
      }
      this.pieChartInstance = new Chart(footChart, {
        type: 'pie',
        data: {
    ...
    createBarChart(data) {
      const barChart = this.qs('#barChart');
      if (!barChart) return;
    ...
    createLineChart(data) {
      const lineCtx = this.qs('#lineChart');
      if (!lineCtx) return;
    ...
    renderUpcomingMaintenance(container, list) {
      ...
      const upcoming = maints
        .map((item) => this.normalizeMaintenance(item))
        .filter((m) => m.date && m.date >= this.todayISO())
    ...
    countUpcoming(list, days) {
      const today = this.todayISO();
      const limit = new Date();
      limit.setDate(limit.getDate() + days);
      const limitStr = limit.toISOString().split('T')[0];
      return (list || [])
        .map((m) => this.normalizeMaintenance(m))
        .filter((m) => m.date && m.date >= today && m.date <= limitStr).length;
    }

    buildMaintenanceTrend(list) {
      const months = [];
      const now = new Date();
      for (let i = 6; i >= 0; i -= 1) {
      ...
      (list || []).forEach((m) => {
        const normalized = this.normalizeMaintenance(m);
  ...
  window.SIGAM_PAGES.DashboardPage = DashboardPage;
})();
