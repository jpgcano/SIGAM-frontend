...
(function () {
    const core = window.SIGAM_CORE || {};
    const endpoints = core.endpoints || {};
    const storage = core.storage || {};
    const apiClient = core.apiClient || {};

    const config = window.SIGAM_CONFIG || {};
    const baseUrl = (endpoints.baseUrl || config.API_BASE_URL || '').replace(/\/+$/, '');
    const ticketsEndpoint = endpoints.tickets || config.TICKETS_ENDPOINT || '/api/tickets';
    const activosEndpoint = endpoints.activos || config.ACTIVOS_ENDPOINT || '/api/activos';
    const repuestosEndpoint = endpoints.repuestos || config.REPUESTOS_ENDPOINT || '/api/repuestos';
    const dashboardEndpoint = endpoints.dashboard || config.DASHBOARD_ENDPOINT || '/api/dashboard';
    const categoriasEndpoint = endpoints.categorias || config.CATEGORIAS_ENDPOINT || '/api/categorias';
    const categoriasTicketEndpoint = endpoints.categoriasTicket || config.CATEGORIAS_TICKET_ENDPOINT || '/api/tickets/categorias';
    const proveedoresEndpoint = endpoints.proveedores || config.PROVEEDORES_ENDPOINT || '/api/proveedores';
    const usuariosEndpoint = endpoints.usuarios || config.USUARIOS_ENDPOINT || '/api/usuarios';
    const mantenimientosEndpoint = endpoints.mantenimientos || config.MANTENIMIENTOS_ENDPOINT || '/api/mantenimientos';

    function getToken() {
        return storage.getToken ? storage.getToken() : localStorage.getItem('sigam_token');
    }
    ...
    function buildQueryString(params = {}) {
        if (apiClient.buildQueryString) return apiClient.buildQueryString(params);
